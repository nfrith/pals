...
End user. Typically uses a Claude harness on a project that happens to be an ALS system. ALS also exposes a Codex install surface now, but full Codex skill/hook/runtime parity is still follow-up work. May not be technical. May never invoke ALS skills directly — they interact with the system through skills that were built for them. Their most common ALS touchpoint is `/run-delamains` to start their pipelines.
...
The interface that discovers, installs, and invokes ALS as a plugin. Today: Claude Code and Codex. Full preview workflow support exists on Claude today; Codex currently ships the install surface only.

### Runtime Harness

The harness-native runtime surface that compiler-owned ALS assets deploy into. Today this remains Claude-only via `alsc deploy claude` and `.claude/`.
...
An item flowing through a delamain (e.g. a `job` in als-factory, a `client` in client-registry). Lives under the module's mounted record root: `<system_root>/<modules.{module_id}.path>/<entities.{entity}.path with placeholders bound>`. For markdown entities, the path template is the identity source of truth: the record frontmatter `id` must match the current entity's `{id}` path binding wherever it appears. Module data records live outside `.als/`. A module version may also declare `ignored_directories` in `module.ts`; those subtrees stay module-owned but are non-record-bearing for validation. The dispatcher reads the record's `status` field to decide which state's agent to run.
...
- **Hooks** — plugin-shipped shell scripts wired into Claude Code's hook lifecycle today. The canonical hook inventory lives in `nfrith-repos/als/.claude-plugin/plugin.json` under `hooks`. Codex hook packaging is a separate follow-up surface.
- **Skills** — bundled skill assets shipped from `nfrith-repos/als/skills/`. Any directory under that tree is part of the bundled skill surface that `alsc deploy claude` projects into `.claude/skills/`. The same tree is packaged by `.codex-plugin/plugin.json` for discovery/install, but functional Codex skill portability is still follow-up work.
...
|----------|----------|----------|
| `<system_root>/.als/` | Operator (authored language) | Modules, delamain definitions, agent prompts, operator-config |
| `<system_root>/<modules.{module_id}.path>/` | Operator (authored data) | Module data records and any mounted record subtree owned by that module |
| `<system_root>/.claude/` | Compiler (projected) | Skill text, hook wiring, deployed assets — regenerated by `alsc deploy claude` today |
...
This boundary is load-bearing: language-upgrade-recipes and construct-upgrade machinery may only mutate `<system_root>/.als/`. Mounted module roots hold the concrete records those authored definitions describe, not more language definitions. The plugin tree is read-only to the operator's system. `.claude/` is regenerated by the compiler, never hand-edited. Codex install-surface metadata now ships from the plugin tree too, but no compiler-owned `.codex/` runtime projection exists yet.
...
| **Plugin version** | `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json` | Vendor release. Rolls up all of the above. | Release act + harness-native plugin refresh |

The plugin version is the rollup that edgerunners see ("you're on plugin 0.2.1"). ALS-097 locks one shared version string across both plugin manifests. The other axes are internal; the engine reconciles them during upgrade flows.
