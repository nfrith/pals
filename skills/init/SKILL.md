---
name: init
description: Create the ALS boot configuration — interactive setup that detects the runtime environment and writes .als/config.md with operator preferences for how delamains should start.
allowed-tools: Bash(bash *)
---

# init

Create the ALS boot configuration for a system. Detects the runtime environment and walks the operator through setting up `.als/config.md` — the file that `/bootup` reads to know how to start delamains.

For the `.als/config.md` format specification, see [`../docs/references/bootup-config.md`](../docs/references/bootup-config.md).

## When to use

- First time setting up an ALS system (no `.als/config.md` exists)
...
```bash
[ -f .als/config.md ] && echo "CONFIG: exists" || echo "CONFIG: none"
```
...
**Options**:
1. **Choose best for me** (Recommended) — Inspect the runtime environment (terminal multiplexer, shell, process manager) and generate a `.als/config.md` with sensible defaults.
2. **Let me describe my setup** — Walk the operator through creating `.als/config.md` interactively.
