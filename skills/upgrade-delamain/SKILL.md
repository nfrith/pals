...
- `preflight`
  ...
  - Otherwise return:
    ...
    - one `confirm-construct-overwrite` prompt for each customized installed dispatcher root
...
- `execute`
  ...
  - Run the `sequential` migration chain across every installed dispatcher root in fleet order.
  - Overwrite vendor-owned dispatcher paths from the canonical bundle, backing up customized installed roots to `<dispatcher>.customized-backup` inside the staging worktree first.
