...
It does not imply that live records or `.als/system.ts` have already been migrated.
...
- `Schema Changes`: flat bullet list of concrete `module.ts` differences.
...
- `Cutover Gates`: flat bullet list of conditions that must be true before `.als/system.ts` can point at `to_version` and the matching future `skills:` set.
...
- `migrate` later copies `skill_paths` into the module's live `.als/system.ts` `skills:` when the cutover lands.
