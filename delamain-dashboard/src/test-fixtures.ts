...
export async function createDashboardFixture(label: string): Promise<DashboardFixture> {
  ...
  await writeFile(
    ...
    [
      ...
      "    phase: implementation",
      "    actor: agent",
      "    label: AI drafted changes for review",
      "    customer_bucket: waiting_for_user",
      "    provider: anthropic",
      "  completed:",
      "    phase: closed",
      "    terminal: true",
  ...
  await writeFile(
    ...
    [
      ...
      "updated: 2026-04-17T10:16:00.000Z",
      "---",
      "",
...
function createItem(
  ...
  return {
    ...
    type,
    filePath: `${root}/${id}.md`,
    title: `${type === "job" ? "Job" : type === "task" ? "Task" : type === "incident" ? "Incident" : "Item"} ${id}`,
    updated: "2026-04-17T10:10:00.000Z",
  };
}
