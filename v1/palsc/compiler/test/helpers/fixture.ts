...
async function createFixtureSandbox(label = "fixture"): Promise<FixtureSandbox> {
  ...
  const root = join(tmpdir(), `pals-compiler-${safeLabel}-${randomUUID()}`);
...
export async function updateSystemYaml(
  ...
  await updateYamlFile(root, ".pals/system.yaml", transform);
}
...
export async function updateShapeYaml(
  ...
  await updateYamlFile(root, `.pals/modules/${moduleId}/v${version}.yaml`, transform);
}
