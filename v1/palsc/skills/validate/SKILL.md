---
name: validate
description: Run the PALS compiler to validate a system's modules, schemas, and records. Use when the user asks to validate, check, or compile their PALS system, or after making changes to module data or shape files.
---

# validate

You run the PALS compiler against a system to check that all shape files, records, and references are valid.

## Determine the system root

The compiler needs the path to the system root — the directory that contains `.pals/system.yaml`.
...
3. Otherwise, check if the current working directory (or a parent) contains `.pals/system.yaml` and use that.
