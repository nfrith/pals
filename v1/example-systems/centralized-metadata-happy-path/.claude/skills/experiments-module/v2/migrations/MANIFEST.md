...
## Shape Changes
- SHP-001: move authoritative deployed metadata from `.pals/modules/experiments/v1.yaml` to `.pals/modules/experiments/v2.yaml`
- SHP-002: entity=experiment; action=add_field; field=budget:number(nullable=true)
- SHP-003: entity=experiment; action=expand_enum; field=status; add=[awaiting-funds,funded]
...
## Cutover Gates
- GATE-001: v2 shape and v2 skill content authored
- GATE-002: migration script executed without failures
- GATE-003: post-migration validation clean
- GATE-004: router and `.pals/system.yaml` deployed pointer updated atomically
