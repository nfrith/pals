...
- `/update` is the sole operator-facing orchestrator. It sequences language-upgrade first when needed, then construct-upgrade preflight, staging, validation, commit, and post-commit lifecycle.
...
- Execute receives the operator answers plus a staging worktree path from `/update`. Execute stages filesystem mutations into that worktree and emits post-commit lifecycle manifests. Execute does not touch the live system directly.
