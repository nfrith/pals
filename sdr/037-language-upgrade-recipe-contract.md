...
- This pass updates the Proposed SDR so the fixture review can focus on authored surface friction instead of reopening already-settled semantics.
...
- The check registry ships in the engine codebase under `nfrith-repos/als/alsc/upgrade/src/checks/`. The initial registry for this job is:
  - `als-version-matches-from`
  - `als-version-matches-to`
  - `validates-as-from-version`
  - `validates-as-to-version`
  - `writes-confined-to-dot-als`
...
- Step-type payloads are:
  - `script` steps: required `path` under `scripts/`, optional `args: string[]`, required `writes: string[]`
  - `agent-task` steps: required `path` under `agent-tasks/`, required `writes: string[]`
...
- The `writes` field declares the full write scope that a mutating step is allowed to touch inside the operator system. Every declared entry is a system-root-relative path or path prefix and must stay under `<system_root>/.als/`.
- `gate` and `operator-prompt` steps must not declare `writes`.
...
- Required: `script` and `agent-task` steps declare their write scope through `writes`.
- Required: every declared `writes` entry stays under `<system_root>/.als/`.
- Required: `gate` and `operator-prompt` steps do not declare writes.
...
- Required: the initial named-check set for this job is `als-version-matches-from`, `als-version-matches-to`, `validates-as-from-version`, `validates-as-to-version`, and `writes-confined-to-dot-als`.
...
- Allowed: `script` or `agent-task` steps to declare `writes: []` when the step is intentionally read-only.
...
- Rejected: write scopes that target `.claude/`, the plugin tree, `CHANGELOG.md`, or any other non-`.als/` surface.
...
- Add recipe parsing and validation for `recipe.yaml`, including:
  ...
  - `.als/`-only validation for `writes`
  - named-check validation for `preconditions` and `postconditions`
  - recovery-routing validation for `recovers`
  - operator-prompt intent validation plus best-effort lint for obviously forbidden prompt text
...
- Add runner support for checkpoint files, chained-hop planning, check-registry execution, write-scope enforcement, recovery dispatch, and structured per-step telemetry.

## Docs and Fixture Impact

- Add `language-upgrades/README.md` to explain the bundle layout, retained fixture rules, and the difference between recipe assets and runner-owned follow-up phases.
...
- In the fixture-first planning pass, paint both allowed and rejected `operator-prompt` examples, named-check usage, and `.als/`-only write scopes before implementation begins.
- `/upgrade-language` must use the full `language-upgrade-recipe` name, the locked schema literals, the named-check vocabulary, and the fix-forward-only recovery model from this SDR.
...
- Module or construct upgrades, including dispatcher version bumps, foundry refreshes, hook-surface migrations, or projection churn outside a language-version hop.
